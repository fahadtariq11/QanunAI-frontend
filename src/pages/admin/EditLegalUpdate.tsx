...
const EditLegalUpdate = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/admin/legal-updates/${id}/`, {
      ...
      .catch(err => {
        console.error(err);
        toast({ title: 'Error', description: 'Failed to load update', variant: 'destructive' });
        setLoading(false);
      });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/legal-updates/${id}/`, {
        method: 'PATCH',
        headers: {
      ...
      toast({ title: 'Success', description: 'Legal update saved successfully.' });
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-heading font-bold text-white">Edit Legal Update</h1>
          <p className="text-slate-400 mt-1">Modify the legal announcement</p>
        </div>
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700 max-w-3xl">
        <CardHeader>
          ...
          <CardDescription className="text-slate-400">
            Update the details for this legal update
          </CardDescription>
        </CardHeader>
        <CardContent>
...
export default EditLegalUpdate;
